...
// JPL_Eph_DE430
//--------------------------------------------------
// Proyecto_TT1: JPL DE430 Ephemerides Implementation
//
...
void JPL_Eph_DE430(double Mjd_TDB, Matrix& r_Mercury, Matrix& r_Venus, Matrix& r_Earth,
    ...
    Matrix Cx_Earth_sub(13, 1);
    Matrix Cy_Earth_sub(13, 1);
    Matrix Cz_Earth_sub(13, 1);
    for (int k = 1; k <= 13; k++) {
        Cx_Earth_sub(k, 1) = Cx_Earth_full(13 * j + k, 1);
        Cy_Earth_sub(k, 1) = Cy_Earth_full(13 * j + k, 1);
        Cz_Earth_sub(k, 1) = Cz_Earth_full(13 * j + k, 1);
    }
    ...
    Mjd0 = t1 + 4 * j;
    Matrix Cx_Moon_sub(13, 1);
    Matrix Cy_Moon_sub(13, 1);
    Matrix Cz_Moon_sub(13, 1);
    for (int k = 1; k <= 13; k++) {
        Cx_Moon_sub(k, 1) = Cx_Moon_full(13 * j + k, 1);
        Cy_Moon_sub(k, 1) = Cy_Moon_full(13 * j + k, 1);
        Cz_Moon_sub(k, 1) = Cz_Moon_full(13 * j + k, 1);
    }
    ...
    temp = temp + 33;
    Cx = Matrix(temp(2, 1) - temp(1, 1), 1);
    Cy = Matrix(temp(3, 1) - temp(2, 1), 1);
    Cz = Matrix(temp(4, 1) - temp(3, 1), 1);
    for (int col = temp(1, 1), idx = 1; col < temp(2, 1); col++, idx++) {
        Cx(idx, 1) = PCtemp(1, col);
    }
    for (int col = temp(2, 1), idx = 1; col < temp(3, 1); col++, idx++) {
    ...
    Matrix Cx_Sun_full(Cx_Sun.n_row + Cx.n_row, 1);
    Matrix Cy_Sun_full(Cy_Sun.n_row + Cy.n_row, 1);
    Matrix Cz_Sun_full(Cz_Sun.n_row + Cz.n_row, 1);
    ...
    for (int k = 1; k <= Cx.n_row; k++) {
        Cx_Sun_full(Cx_Sun.n_row + k, 1) = Cx(k, 1);
        Cy_Sun_full(Cy_Sun.n_row + k, 1) = Cy(k, 1);
        Cz_Sun_full(Cz_Sun.n_row + k, 1) = Cz(k, 1);
    }
    if (0 <= dt && dt <= 16) {
        j = 0;
        Mjd0 = t1;
    ...
    Matrix Cx_Sun_sub(11, 1);
    Matrix Cy_Sun_sub(11, 1);
    Matrix Cz_Sun_sub(11, 1);
    for (int k = 1; k <= 11; k++) {
        Cx_Sun_sub(k, 1) = Cx_Sun_full(11 * j + k, 1);
        Cy_Sun_sub(k, 1) = Cy_Sun_full(11 * j + k, 1);
        Cz_Sun_sub(k, 1) = Cz_Sun_full(11 * j + k, 1);
    }
    ...
    Mjd0 = t1 + 8 * j;
    Matrix Cx_Mercury_sub(14, 1);
    Matrix Cy_Mercury_sub(14, 1);
    Matrix Cz_Mercury_sub(14, 1);
    for (int k = 1; k <= 14; k++) {
        Cx_Mercury_sub(k, 1) = Cx_Mercury_full(14 * j + k, 1);
        Cy_Mercury_sub(k, 1) = Cy_Mercury_full(14 * j + k, 1);
        Cz_Mercury_sub(k, 1) = Cz_Mercury_full(14 * j + k, 1);
    }
    ...
    Matrix Cx_Venus_full(Cx_Venus.n_row + Cx.n_row, 1);
    Matrix Cy_Venus_full(Cy_Venus.n_row + Cy.n_row, 1);
    Matrix Cz_Venus_full(Cz_Venus.n_row + Cz.n_row, 1);
    ...
    for (int k = 1; k <= Cx.n_row; k++) {
        Cx_Venus_full(Cx_Venus.n_row + k, 1) = Cx(k, 1);
        Cy_Venus_full(Cy_Venus.n_row + k, 1) = Cy(k, 1);
        Cz_Venus_full(Cz_Venus.n_row + k, 1) = Cz(k, 1);
    }
    if (0 <= dt && dt <= 16) {
        j = 0;
        Mjd0 = t1;
    ...
    Matrix Cx_Venus_sub(10, 1);
    Matrix Cy_Venus_sub(10, 1);
    Matrix Cz_Venus_sub(10, 1);
    for (int k = 1; k <= 10; k++) {
        Cx_Venus_sub(k, 1) = Cx_Venus_full(10 * j + k, 1);
        Cy_Venus_sub(k, 1) = Cy_Venus_full(10 * j + k, 1);
        Cz_Venus_sub(k, 1) = Cz_Venus_full(10 * j + k, 1);
    }
    ...
    j = 0;
    Mjd0 = t1;
    Matrix Cx_Mars_sub(11, 1);
    Matrix Cy_Mars_sub(11, 1);
    Matrix Cz_Mars_sub(11, 1);
    for (int k = 1; k <= 11; k++) {
        Cx_Mars_sub(k, 1) = Cx_Mars(k, 1);
        Cy_Mars_sub(k, 1) = Cy_Mars(k, 1);
        Cz_Mars_sub(k, 1) = Cz_Mars(k, 1);
    }
    ...
    j = 0;
    Mjd0 = t1;
    Matrix Cx_Jupiter_sub(8, 1);
    Matrix Cy_Jupiter_sub(8, 1);
    Matrix Cz_Jupiter_sub(8, 1);
    for (int k = 1; k <= 8; k++) {
        Cx_Jupiter_sub(k, 1) = Cx_Jupiter(k, 1);
        Cy_Jupiter_sub(k, 1) = Cy_Jupiter(k, 1);
        Cz_Jupiter_sub(k, 1) = Cz_Jupiter(k, 1);
    }
    ...
    j = 0;
    Mjd0 = t1;
    Matrix Cx_Saturn_sub(7, 1);
    Matrix Cy_Saturn_sub(7, 1);
    Matrix Cz_Saturn_sub(7, 1);
    for (int k = 1; k <= 7; k++) {
        Cx_Saturn_sub(k, 1) = Cx_Saturn(k, 1);
        Cy_Saturn_sub(k, 1) = Cy_Saturn(k, 1);
        Cz_Saturn_sub(k, 1) = Cz_Saturn(k, 1);
    }
    ...
    j = 0;
    Mjd0 = t1;
    Matrix Cx_Uranus_sub(6, 1);
    Matrix Cy_Uranus_sub(6, 1);
    Matrix Cz_Uranus_sub(6, 1);
    for (int k = 1; k <= 6; k++) {
        Cx_Uranus_sub(k, 1) = Cx_Uranus(k, 1);
        Cy_Uranus_sub(k, 1) = Cy_Uranus(k, 1);
        Cz_Uranus_sub(k, 1) = Cz_Uranus(k, 1);
    }
    ...
    j = 0;
    Mjd0 = t1;
    Matrix Cx_Neptune_sub(6, 1);
    Matrix Cy_Neptune_sub(6, 1);
    Matrix Cz_Neptune_sub(6, 1);
    for (int k = 1; k <= 6; k++) {
        Cx_Neptune_sub(k, 1) = Cx_Neptune(k, 1);
        Cy_Neptune_sub(k, 1) = Cy_Neptune(k, 1);
        Cz_Neptune_sub(k, 1) = Cz_Neptune(k, 1);
    }
    ...
    j = 0;
    Mjd0 = t1;
    Matrix Cx_Pluto_sub(6, 1);
    Matrix Cy_Pluto_sub(6, 1);
    Matrix Cz_Pluto_sub(6, 1);
    for (int k = 1; k <= 6; k++) {
        Cx_Pluto_sub(k, 1) = Cx_Pluto(k, 1);
        Cy_Pluto_sub(k, 1) = Cy_Pluto(k, 1);
        Cz_Pluto_sub(k, 1) = Cz_Pluto(k, 1);
    }
    ...
    Mjd0 = t1 + 8 * j;
    Matrix Cx_Nutations_sub(10, 1);
    Matrix Cy_Nutations_sub(10, 1);
    for (int k = 1; k <= 10; k++) {
        Cx_Nutations_sub(k, 1) = Cx_Nutations_full(10 * j + k, 1);
        Cy_Nutations_sub(k, 1) = Cy_Nutations_full(10 * j + k, 1);
    }
    Matrix Nutations = Cheb3D(Mjd_TDB, 10, Mjd0, Mjd0 + 8, Cx_Nutations_sub, Cy_Nutations_sub, zeros(10, 1));
    ...
    Mjd0 = t1 + 8 * j;
    Matrix Cx_Librations_sub(10, 1);
    Matrix Cy_Librations_sub(10, 1);
    Matrix Cz_Librations_sub(10, 1);
    for (int k = 1; k <= 10; k++) {
        Cx_Librations_sub(k, 1) = Cx_Librations_full(10 * j + k, 1);
        Cy_Librations_sub(k, 1) = Cy_Librations_full(10 * j + k, 1);
        Cz_Librations_sub(k, 1) = Cz_Librations_full(10 * j + k, 1);
    }
